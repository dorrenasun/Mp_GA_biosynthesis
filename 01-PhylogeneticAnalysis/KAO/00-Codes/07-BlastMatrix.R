...
#Load sequences to check the length
file.fasta<-list.files(outdir,"05-Subtree.fasta",full.names = T)
fasta<-read_fasta(file.fasta)
fasta[,Seqlen:=nchar(Sequence)]
...
blast.sele[,`:=`(qname=annot_sub[match(qaccver,Num),label],
                 ...
                 x1=annot_sub[match(qaccver,Num),No] + (annot_sub[match(qaccver,Num),Group]-1)*gap1,
                 # xg=,
                 sname=annot_sub[match(saccver,Num),label],
                 y=annot_sub[match(saccver,Num),No] + (annot_sub[match(saccver,Num),Group]-1)*gap,
                 y1=annot_sub[match(saccver,Num),No] + (annot_sub[match(saccver,Num),Group]-1)*gap1
...
blast.sele.m<-blast.sele[,.(qname,sname,x,y,x1,y1,pident)]
...
theme_hm<-theme_bw()+theme(aspect.ratio = 1,
                           ...
                           axis.ticks = element_line(linewidth = lwd*0.5,color="black"),
...
p1<-ggplot(msa.sele,aes(x=x,y=y,fill=pident))+
  ...
  theme_hm#+
ggsave(paste0(outdir,"08-PerIdent_matrix-msa.pdf"),p1,width = fw,height = fh,units = "cm")
...
print(p2)
ggsave(paste0(outdir,"08-PerIdent_matrix-blast.pdf"),p2,width = fw,height = fh,units = "cm")
...

{
  #Labels for p3
  grp.sele<-c(1,3,5,6)
  labs<-annot_sub[Group %in% grp.sele,.(breaks=mean(No)+(Group-1)*gap1),by=.(Group)]
  labs[,label:=c("Outgroup","CYP817","CYP729","CYP88\n(KAO)")]
  
  p3<-ggplot(blast.sele.m,aes(x=x1,y=y1,fill=pident))+
    geom_tile()+
    scale_x_continuous(position = "top",
                       expand = expansion(mult=0,add=0.5),
                       breaks = labs$breaks,
  ...
  ggsave(paste0(outdir,"08-PerIdent_matrix-blast-small.pdf"),p3,width = 4,height = 6,units = "cm")
