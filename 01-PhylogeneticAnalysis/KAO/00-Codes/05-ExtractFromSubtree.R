...
# subtree<-"../01-Input/SubTree-CKI1.nex"
# subtree<-"../01-Input/SubTree.nex"
...
# #Find outgroups from blast results
# tsv.file<-"../02-Output/01-AnnotatedGenomes-blast.tsv"
# tsv<-fread(tsv.file,header = F)
# names(tsv)<-unlist(strsplit("qaccver saccver pident length mismatch gapopen qstart qend sstart send evalue bitscore ppos",split=" "))
# tsv.AtMp<-tsv[grep("AT.G.*|Mp.g.*",saccver),]
# tsv.AtMp[grep("AT.G.*",saccver),Sp:="At"]
# tsv.AtMp[grep("Mp.g.*",saccver),Sp:="Mp"]
# #Keep only primary
# tsv.AtMp[,Gene:=sub("\\..*","",saccver)]
# tsv.AtMp[,var:=as.integer(sub(".*\\.","",saccver))]
# setkeyv(tsv.AtMp,c("qaccver","Gene","var"))
# tsv.AtMp.pri<-tsv.AtMp[!duplicated(Gene),.(saccver,evalue,Sp),by=c("qaccver","Gene")]
# tsv.AtMp.out<-tsv.AtMp.pri[(!saccver %in% all_seqs$ID_sep),]
# setkeyv(tsv.AtMp.out,c("Sp","evalue"))
# tsv.AtMp.out[,rank:=seq(1:.N),by=Sp]
# tsv.out<-tsv.AtMp.out[rank<=3,saccver]
# message("Outgroups from blast top ranks:")
# message(paste0(tsv.out,"\n"))
# 
#Add outgroup sequences from file
outgroup<-fread("../01-Input/Outgroup.txt",header = F,sep = "\t",strip.white = T)
...
#Remove outliers
suppressWarnings(
  exclude<-fread("../01-Input/Exclude.txt",header = F,sep = "\t")
...
# write(fasta.sub$Text,paste0(outdir,"05-Subtree-CKI1.fasta"))
# write(fasta.sub$Textnew,paste0(outdir,"05-Subtree-CKI1_for_human.fasta"))
