...
pkg_list<-c("data.table","ggplot2","ggtree","treeio","tidytree","ggnewscale")
for (p in pkg_list) {check_package(p)}

if (!exists("read_fasta", mode="function")|!exists("read_hmm", mode="function")) source("00-Functions.R")
#================================================================================================
# OutDir<-file.path(outdir,"05-Subtree-Round5")
OutDir<-outdir
#Inputs & outputs
tree.file<-list.files(path = OutDir,pattern = ".*07.*np.*ordered.newick",full.names = T)
subname.file<-list.files(path = "../01-Input/",pattern = "Name_sub",full.names = T)

if (file.copy(subname.file,OutDir)) message(paste0(subname.file,"copied to",OutDir))
...
# font_add("Arial",regular = "Arial.ttf", italic = "Arial Italic.ttf", bold = "Arial Bold.ttf")
# showtext_auto()
fontfam<-"sans"
...
annot_sub[,Full_Name:=paste0("plain('[')*italic('",Species,"')*plain('] ",label,"')"),by=.(No)]
...
{ #Extract various nodes
  ...
  nodes<-data.table(node=c(node.KAO,node.CYP729,node.CYP817,node.outgroup),
                    isKAO=c("T","F","F","F"),
                    label=c("CYP88 (KAO)","CYP729","CYP817","Outgroup")
                    )
}
...
p1 <- ggtree(tree2, ladderize = F, color="black",size=0.5*lwd) +
  ...
  geom_hilight(data = nodes,
               mapping=aes(node=node,fill=isKAO),
               type="gradient",
               ...
               extendto=xlim-0.05,
               to.bottom = T) +
  scale_fill_manual(values = c("T"="#bbc6d9","F"="grey70"),guide="none")+
  ...
  theme(
    ...
    #   face = "plain"
    # ),
    legend.text = element_text(size=fontsize,color = "black",family = fontfam),
    legend.title = element_text(size=fontsize,color = "black",family = fontfam),
...
ggsave(outfile1,p1,width = 16, height = 24,units = "cm")

# Cairo(file=outfile1,width = 16, height = 22,units = "cm",type="pdf",family="Arial")
# plot(p1)
# dev.off()
# ggsave(outfile1,p1,width = 16, height = 24,units = "cm",device = cairo_pdf,dpi = 600)
# outfile2<-file.path(OutDir,sub("newick","svg",basename(tree.file)))
# ggsave(outfile2,p1,width = 16, height = 22,units = "cm",device = svg)
